...
countries.sort()
cont_name_len = [len(c) for c in countries]
max_len = max(cont_name_len)
long_names = [c for c in countries if len(c) == max_len]
shor_name = [c for c in countries if len(c) == min(cont_name_len)]
print(f'Short Name Countries: {shor_name}\nLong Name Countries: {long_names}')
